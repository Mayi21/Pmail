...
本文档面向想把 PMail 部署到自己 Cloudflare 账号的开发者，覆盖**本地一键部署**与 **GitHub Actions 自动部署**两条路径，从零环境准备一直到日常运维、备份恢复、卸载与排错。
...
仓库内提供了一键脚本 `scripts/bootstrap.mjs`（Node 18+），一次执行内完成 D1 / R2 / KV / Pages 资源创建，自动检测已存在的资源并**幂等复用**，最后渲染本地 `wrangler.toml` 系列文件以及 `.env`。
...
- **Worker secrets**：通过 `wrangler secret put` 推送到 Cloudflare，运行时由 Worker 读取（本地部署与 CI 部署都需要）
- **GitHub Secrets**：仅当走 GitHub Actions 自动部署时录入，CI 用 `envsubst` 渲染 `wrangler.toml` 并镜像推送 Worker secrets

### 3.1 Worker secrets（本地与 CI 都需要）
...
| `KV_JWT_KEYS_ID` | JWT_KEYS 命名空间 ID | [§2.2](#22-手动方式逐条-wrangler-命令) |
...
```bash
# 重跑 bootstrap 让 wrangler.toml 的 [vars] 也填上
# 这次脚本会看到资源已存在（reuse），但会用 .env 里的新值重新渲染 wrangler.toml
node scripts/bootstrap.mjs

# 部署（按需选择其一）
node scripts/bootstrap.mjs --deploy     # bootstrap + 部署一条龙
# 或者
bash deploy.sh                          # 调用三个 Worker 各自的 wrangler deploy
```

部署完成后控制台输出每个 Worker 的访问 URL，记下 API Worker 的 URL（形如 `https://pmail-api.<your-subdomain>.workers.dev`），下一步要用。
...
6. 回到 Cloudflare Dashboard 按 [§5](#5-email-routing-绑定最后一步必须人工) 绑定 Email Routing。
...
> 必须先执行了 [§4](#4-部署) 让 `pmail-receiver` 部署成功，下拉框里才能选到。
...
**`[vars]` 类**（如 `ALLOWED_ORIGINS`、`KEY_ROTATION_DAYS`、`DOMAIN`）
...
| Email Worker 部署成功但收不到邮件 | 未在 Dashboard 配置 Email Routing 路由 | 按 [§5](#5-email-routing-绑定最后一步必须人工) 绑定 catch-all → `pmail-receiver` |
| deploy 报 "缺少 workers/api/wrangler.toml" | 没跑过 `bootstrap.mjs`，或 `.env` 里 envsubst 变量未填全导致渲染留有 `${VAR}` 占位符 | 检查 `.env`，把 `DOMAIN` / `ALLOWED_ORIGINS` 补上后重跑 `node scripts/bootstrap.mjs` |
...
2. 是否绑定了 catch-all → `pmail-receiver` 路由（[§5](#5-email-routing-绑定最后一步必须人工)）

...
- 邮件实时接收、搜索、附件下载
- JWT 静态密钥（HS256）、登录失败锁定、双层速率限制等安全特性
...
1. 首页点击"注册"，填邮箱（仅作账号标识，不会用来收信）+ 密码（8–64 位，含大小写字母 + 数字）
2. 登录后可以：
...
可选但推荐：
- 自定义前端 / API 域名（避免使用默认的 `*.pages.dev` / `*.workers.dev`）
...
# 脚本会：
#   - 自动创建/复用 D1（pmail-db）、R2（pmail-storage）、KV（CACHE）、Pages（pmail-web）
#   - 渲染 workers/api/wrangler.toml、workers/email/wrangler.toml
#   - 写一份 .env，把资源 ID 和 Pages 项目名填进去
#   - 自动启用 Cloudflare Email Routing（MX / SPF 记录），并生成 + 推送 JWT_SECRET（已存在则跳过）
...
# 3. Worker 运行时 secrets：JWT_SECRET 已由 bootstrap 自动推送
#    可选 secret 见 docs/DEPLOYMENT.md §3.1（如启用 admin 域名/转发管理用的 CLOUDFLARE_API_TOKEN）
...
| [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) | 部署完整指南：bootstrap.mjs 初始化 + GitHub Actions、Secrets 清单、排错 |

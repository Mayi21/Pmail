...
- **API Worker** (`workers/api/`) — Hono REST API，JWT/API Key 认证、用户/邮箱/邮件管理、定时任务、Queue 消费者
...
数据存储：D1（SQLite）、R2 单桶（附件用 `attachments/` 前缀，数据库备份用 `backups/` 前缀）、2 个 KV namespace：`JWT_KEYS`（版本化 JWT 签名密钥）和 `CACHE`（共享缓存，按 key 前缀复用：`reset:*` 密码重置 / `oauth:*` OAuth state / `email_valid:*` 收件地址正向缓存 / `settings:*` 应用级配置）。

## 架构原则

### 数据所有权
所有用户数据通过外键链定位：`users → temp_emails → emails → attachments`。**所有查询必须包含 `user_id` 条件**（游客邮箱例外，见下文）。中间件 `workers/api/src/middleware/auth.ts` 从 JWT 或 API Key 解析 `user_id` 并写入 Hono context。

### 三种身份
1. **注册用户** — JWT (`/api/*` 路径) 或 OAuth (Linux.do)。OAuth 账户的 `password_hash` 为 NULL，与密码账户隔离
2. **API Key 用户** — `X-API-Key` header (`/v1/*` 路径)，SHA-256 哈希存储在 `api_keys` 表，支持每用户多 key、`permissions` (`read,write` CSV)、`expires_at`、`is_active`
3. **游客** — `temp_emails.user_id IS NULL`，2 小时 TTL；查询模式 `WHERE address = ? AND user_id IS NULL`

### JWT 密钥轮换
密钥版本化存储在 `JWT_KEYS` KV，token header 携带 `kid`。`active-keys` key 维护当前活跃密钥 ID 列表。验证时按 `kid` 查找并检查状态（active/retired/deleted）。轮换由每日备份 cron `0 2 * * *` 的 scheduled handler 判定：当 `day-of-month % KEY_ROTATION_DAYS === 0` 时调用 `services/jwtKeyManager.ts:rotateJWTKeys`。
...
3. `postal-mime` 解析；若设置了 `DATABASE_ENCRYPTION_KEY`，正文/HTML/raw 走 AES-256 加密 (`workers/email/src/utils/crypto.ts`)
...
- `/api/*` — 前端使用，JWT 认证
- `/v1/*` — 北向 API，API Key 认证（部分路由文件同时含 `/api` 和 `/v1` 两条路径，所以会在根上 `app.route('/', mailboxRoutes)` 挂载一次）
...
```bash
cd web
npm run dev              # localhost:5173（已通过 vite.config.ts 代理 /api 和 /v1 到 :8787）
...
| `0 2 * * *` | 每日 D1 备份到 R2（`backups/` 前缀）+ 按 `KEY_ROTATION_DAYS` 判定是否轮换 JWT 密钥 | `performDatabaseBackup` + `cleanupOldBackups` + 条件触发 `rotateJWTKeys` |
...
6. **JWT key rotation** — 测试轮换时必须保留 `KEY_GRACE_PERIOD_DAYS` 重叠期，否则旧 token 立即失效
7. **OAuth 与密码账户** — OAuth 用户 `password_hash` 为 NULL，登录/重置流程要单独处理
8. **多 API Key** — `api_keys.user_id` **不再** 唯一；查询时不要假设一对一
9. **CORS** — `ALLOWED_ORIGINS` 是逗号分隔字符串，`workers/api/src/index.ts` 中有正则匹配 Cloudflare Pages preview 域名
...
| JWT 密钥管理 | `workers/api/src/services/jwtKeyManager.ts` |
| 速率限制 | `workers/api/src/middleware/rateLimit.ts` |
| 邮件接收 | `workers/email/src/index.ts` |
| 邮件加密 | `workers/email/src/utils/crypto.ts` |
| 数据库 schema | `schema.sql` |
| 增量迁移 | `migrations/` |
| OpenAPI 规范 | `workers/api/openapi.yaml` |
...
- `docs/SECURITY_IMPROVEMENT_PLAN.md` — 安全增强路线图
- `docs/PRODUCTION_CHECKLIST.md` — 上线检查清单
- `docs/other.md` — API 接口与架构原理（含完整端点列表）
- `README.md` — 部署步骤与功能介绍
